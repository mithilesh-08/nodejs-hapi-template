...
import { findAllVehiclesByDriverId } from '@daos/vehicleDao';
  ...
  },
  {
    ...
    handler: async (request) => {
      try {
        ...
        const include = [
          { model: models.roles, attributes: ['name'], as: 'role' },
        ];
        ...
        // If user is a driver, fetch their vehicle details
        let vehicles;
        if (user.role.name === 'DRIVER') {
          vehicles = await findAllVehiclesByDriverId(user.id, 1, 5);
        }
