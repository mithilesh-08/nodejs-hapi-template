...
describe('pricing config daos', () => {
  ...
  describe('findAllPricingConfigs', () => {
    ...
    it('should find all pricing configs', async () => {
      ...
      await resetAndMockDB((db) => {
        jest
          .spyOn(db.models.pricingConfigs, 'findAndCountAll')
          .mockImplementation(() =>
            Promise.resolve({
              count: 1,
              rows: [
      ...
      const { pricingConfigs, total } = await findAllPricingConfigs(
        page,
        limit,
      );
    ...
    it('should call findAndCountAll with the correct parameters', async () => {
      await resetAndMockDB((db) => {
        spy = jest.spyOn(db.models.pricingConfigs, 'findAndCountAll');
      });
